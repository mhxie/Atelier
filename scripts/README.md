...
| `routine_digest.py` | Rolls scheduled-routine outputs plus optional append-only status ledgers from `$OV/_meta/digest_updates.toml` into one daily/weekly document. Routine files use window or unacked selection; mandatory status rows render deterministically before the overview, use a delivery-only cursor for the next daily artifact, and repeat in the weekly window. Writes semantic HTML into the routine's declared `$OV` output directory and can advance review acknowledgments separately; subcommands `collect`, `render`, `write`, `ack`. Nothing generated here enters Readwise | ops | stdlib |
| `daily_context.py` | Masthead context for the daily digest: each coding harness's remaining weekly window read from files it already leaves behind (claude-hud usage snapshot, Codex session `rate_limits` events) with a countdown to reset and the snapshot age, plus the day's forecast from Open-Meteo for a `--place` the digest procedure takes from the calendar. Quota never touches the network; weather is the only fetch and degrades to a warning | ops | stdlib |

...
| `privacy_index.py` | Private-entity index at `$OV/_meta/privacy_index.json`: directory names and paths, note stems, wikilinks, routine and feature registries, frontmatter, profile proper nouns, each with provenance; `build`, `why`, `stats` | ops | stdlib |
| `privacy_check.py` | Mechanical privacy gate: scans public-bound pathnames, worktree content, divergent staged blobs, or a history range (`--range A..B`) against the private-entity index (term rule + path rule); hits carry provenance (`--why`); deliberate public opt-outs live in `privacy_allowlist.txt`; wired into `/lint`, `/system-review`, `/push`, and `hooks/pre-push` | ops | stdlib |
...
| `routine_digest.py` | Digest CLI (`collect` / `render` / `write` / `check` / `ack` / `mail` / `report`) and the facade that re-exports the pipeline: `routine_digest_core.py` (registries, windows, markdown and manifest helpers), `routine_collect.py` (collection with a one-day carry of undelivered files and the delivered ledger, updates, health, context), `routine_render.py` (mail-safe HTML), `routine_mail.py` (SMTP delivery) | ops | stdlib |
...
| `autoevo_verify.py` | Proves one autoevo cycle delivered a committed audit, left no bot-owned dirt, and had real Forgetter coverage, one same-commit decay report per returned envelope, lint, claim-owned event journal, and matching final verification evidence | ops | stdlib, `git` |
